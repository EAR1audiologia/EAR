...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--color-border)] bg-[color:var(--color-bg)]/92 backdrop-blur">
        ...
        <div className="flex items-center justify-between py-3">
          <div className="flex items-center gap-6 xl:gap-8">
            ...
            <nav className="hidden lg:flex items-center gap-7 xl:gap-8">
              <Link
                href="/servicios"
                className="relative py-1 text-sm font-medium text-[var(--color-ink)] transition-colors hover:text-[var(--color-brand-strong)] after:absolute after:left-0 after:-bottom-0.5 after:h-0.5 after:w-0 after:bg-[var(--color-brand-strong)] after:transition-all after:duration-300 hover:after:w-full"
              >
                Servicios
              </Link>
              <Link
          ...
          <div className="flex items-center gap-3 xl:gap-4 pl-4">
            <MobileNav />
            <a
              href={`https://wa.me/${whatsappNumber}?text=${whatsappText}`}
              className="hidden rounded-full border border-[var(--color-border)] bg-white px-5 py-2 text-sm font-semibold shadow-sm hover:bg-zinc-50 sm:inline-flex items-center gap-2"
              target="_blank"
              rel="noopener noreferrer"
            >
            ...
            <a
              ...
              className="hidden rounded-full bg-[var(--color-accent)] px-5 py-2 text-sm font-semibold text-white shadow-sm hover:opacity-95 sm:inline-flex items-center gap-2"
            >
            ...
            <a
              href={`tel:${siteConfig.phone.landline}`}
              className="inline-flex rounded-full border border-[var(--color-border)] bg-white px-4 py-2 text-sm font-semibold hover:bg-zinc-50 sm:hidden items-center gap-2"
            >
              <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75 6 3a.75.75 0 0 1 .85-.16l2.25 1.125a.75.75 0 0 1 .39.636v1.716a.75.75 0 0 1-.21.53l-.84.84a.75.75 0 0 0-.218.466 12.06 12.06 0 0 0 6.25 6.25.75.75 0 0 0 .466-.217l.84-.84a.75.75 0 0 1 .53-.211h1.716a.75.75 0 0 1 .636.391l1.125 2.25A.75.75 0 0 1 21 18l-3.75 3.75-.472 2.028Z" />
              </svg>
